...
export function WorkspaceProvider({ children }) {
  ...
  useEffect(() => {
    const handleOnline = () => {
      setIsOnline(true);
      addToast({ title: "Connection Restored", description: "You are back online. Offline changes synced.", type: "info" });
    };
    const handleOffline = () => {
      setIsOnline(false);
  ...
  const addWorkspace = useCallback((wsData) => {
    const newWs = {
      id: `ws-${Date.now()}`,
    ...
    // Create a default project inside
    const defaultProj = {
      id: `proj-${Date.now()}`,
  ...
  const deleteWorkspace = useCallback((id) => {
    ...
    // Also clean up tasks belonging to projects in this workspace
  ...
  // Project Operations
  const addProject = useCallback((projData) => {
    ...
    // If template has default tasks, populate them
  ...
  const setProjectViewPreference = useCallback((projectId, view) => {
    setProjects(prev => prev.map(p => p.id === projectId ? { ...p, viewPreference: view } : p));
  }, [setProjects]);

  // Column Operations
  const addColumn = useCallback((projectId, columnName) => {
  ...
  const renameColumn = useCallback((projectId, oldName, newName) => {
    ...
    // Also update all tasks that were in old column
  ...
  const deleteColumn = useCallback((projectId, columnName) => {
    ...
    // Reassign tasks in this column to fallback
    setTasks(prev => prev.map(t => {
  ...
  const convertSubtaskToTask = useCallback((taskId, subtaskId) => {
  ...
  const convertTaskToSubtask = useCallback((parentTaskId, taskIdToConvert) => {
    ...
    // Add subtask to parent & remove child task
    setTasks(prev => prev
  ...
  // Bulk Operations
  const bulkUpdateStatus = useCallback((taskIds, newStatus) => {
  ...
  const bulkUpdatePriority = useCallback((taskIds, newPriority) => {
  ...
  const bulkAssign = useCallback((taskIds, userIds) => {
  ...
  const syncChanges = useCallback(() => {
    ...
    setTimeout(() => {
      setIsSyncing(false);
      addToast({ title: "Sync Complete", description: "All local changes synchronized with cloud storage.", type: "info" });
    }, 1200);
  }, [addToast]);
  ...
  // Simulated Live Activity (background events from other team members)
  useEffect(() => {
